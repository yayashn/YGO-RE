...
import { get3DZone, createInstance } from "shared/utils";
...
Remotes.Client.OnEvent("createCard3D", (card2D: SurfaceGui, location: Location, isOpponent) => {
    ...
    const card3DValue = createInstance("ObjectValue", "card", card3D) as ObjectValue;
    card3DValue.Value = card3D;
    const card2DValue = createInstance("ObjectValue", "card2D", card3D) as ObjectValue;
    card2DValue.Value = card2D;
    const orderValue = createInstance("IntValue", "order", card3D) as IntValue;
    orderValue.Value = 0;
 ...
 //   status.Adornee = card3D;
    ...
    const connection = cardClickDetector.MouseClick.Connect(() => {
        const onCardClick = card2D.FindFirstChild("onCardClick") as RemoteEvent;
