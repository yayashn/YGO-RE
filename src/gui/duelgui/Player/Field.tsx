...
const FieldZoneButton = withHooks(({zoneName, layoutOrder, animate, useSelected: [selected, setSelected]}: FieldZoneButtonProps) => {
...
export default withHooks(() => {
    ...
    const fieldZones: FieldZone[] = ["MZone1", "MZone2", "MZone3", "MZone4", "MZone5", "SZone1", "SZone2", "SZone3", "SZone4", "SZone5"];
    ...
    return (
        <surfacegui Key="FieldPlayer" Face="Top" Adornee={FieldPlayer}>
            <uigridlayout SortOrder="LayoutOrder" CellPadding={new UDim2(0,0,0.005,0)} CellSize={new UDim2(0.2,0,0.5,0)}/>
            {fieldZones
            .map((_, i, zonesArray) => {
                const zone = zonesArray[(zonesArray.size() - i - 1)]
                return <FieldZoneButton useSelected={[selected, setSelected]} animate={animateFieldZones.find((n: string) => n === zone) !== undefined} zoneName={zone} layoutOrder={i}/>
            })}
        </surfacegui>
    )
})